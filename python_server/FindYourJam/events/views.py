...
def create_event(request):
    ...
    event = Event(title=request_data.get('title'),
                  ...
                  lat=request_data.getInt('lat'),
                  long=request_data.getInt('long'),
...
def get_multiple_events(request):
    ...
    if not (request_data.get('lat') and request_data.get('long') and
        ...
        return Response(data=response_data, status=status.HTTP_400_BAD_REQUEST)
    lat_req = request_data.getInt('lat')
    long_req = request_data.getInt('long')
    radius_req = request_data.getInt('radius')
